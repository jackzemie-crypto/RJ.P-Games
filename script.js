...
function displayMovies(movieList) {
    ...
    movieList.forEach((movie, index) => {
        ...
        img.onerror = function() {
            // Create a styled fallback with movie info
            ...
            this.alt = movie.name + ' (' + movie.year + ')';
            // Use a data URL as fallback
            this.src = 'data:image/svg+xml,' + encodeURIComponent(
                '<svg xmlns="http://www.w3.org/2000/svg" width="300" height="450">' +
                '<rect width="300" height="450" fill="#1a1a1a"/>' +
                '<text x="50%" y="50%" text-anchor="middle" fill="#ffffff" font-size="16" font-family="Arial">' +
                movie.name.substring(0, 30) + (movie.name.length > 30 ? '...' : '') +
                '</text>' +
                '<text x="50%" y="60%" text-anchor="middle" fill="#999999" font-size="14" font-family="Arial">(' + movie.year + ')</text>' +
                '</svg>'
            );
        };
